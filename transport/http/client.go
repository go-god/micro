...
func NewClient(method string, tgt *url.URL, enc EncodeRequestStream, dec DecodeResponseStream, options ...ClientOption) *Client {
...
func NewExplicitClient(req CreateRequestStream, dec DecodeResponseStream, options ...ClientOption) *Client {
	c := &Client{
...
// Endpoint returns a usable micro endpoint that calls the remote HTTP endpoint.
func (c Client) Endpoint() endpoint.Endpoint {
	return func(ctx context.Context, request interface{}) (interface{}, error) {
		...
		req, err := c.req.CreateRequest(ctx, request)
		if err != nil {
			cancel()
			return nil, err
		}
		...
		response, err := c.dec.DecodeResponse(ctx, resp)
		if err != nil {
			return nil, err
		}
...
func makeCreateRequestFunc(method string, target *url.URL, enc EncodeRequestStream) CreateRequestStream {
	return CreateRequestFunc(func(ctx context.Context, request interface{}) (*http.Request, error) {
		req, err := http.NewRequest(method, target.String(), nil)
		if err != nil {
			return nil, err
		}

		if err = enc.EncodeRequest(ctx, req, request); err != nil {
			return nil, err
		}
