...
func (s Server) ServeGRPC(ctx context.Context, req interface{}) (retctx context.Context, resp interface{}, err error) {
	...
	request, err = s.decodeRequestStream.DecodeRequest(ctx, req)
	if err != nil {
	...
	grpcResp, err = s.encodeResponseStream.EncodeResponse(ctx, response)
	if err != nil {
		s.errorHandler.Handle(ctx, err)
		return ctx, nil, err
	}
...
// context so it can be consumed by micro gRPC middlewares. The Interceptor
// typically is added at creation time of the grpc-go server.
// Like this: `grpc.NewServer(grpc.UnaryInterceptor(grpc.Interceptor))`
func Interceptor(
...
func StreamServerInterceptor(
	...
	ctx = context.WithValue(ctx, ContextKeyRequestMethod, info.FullMethod)
	ctx = context.WithValue(ctx, ContextKeyIsClientStream, info.IsClientStream)
	ctx = context.WithValue(ctx, ContextKeyIsServerStream, info.IsServerStream)
